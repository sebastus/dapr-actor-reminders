...
namespace MyActorService
{
    internal class MyActor : Actor, IMyActor, IRemindable
    {
        ...
        public async Task ReceiveReminderAsync(string reminderName, byte[] state, TimeSpan dueTime, TimeSpan period)
        {
            ...
            switch (reminderName)
            {
                case "MyFirstReminder":
                    {
                        if (data.CountOfReminder++ > 2)
                        {
                            data.CountOfReminder = 0;
                            
                            await this.UnregisterReminder("MyFirstReminder");

                            //await this.RegisterReminder("MySecondReminder");
                        }
                        break;
                    }

                case "MySecondReminder":
                    {
                        if (data.CountOfReminder++ > 2)
                        {
                            data.CountOfReminder = 0;
                            
                            await this.UnregisterReminder("MySecondReminder");
                        }
                        break;
                    }
            }
